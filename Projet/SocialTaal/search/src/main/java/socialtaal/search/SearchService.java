...
public class SearchService {
    ...
    public List<User> searchUsers(String pseudo, String gender, Integer minAge, Integer maxAge, String birthCountry, String motherTongue) {
        ...
        Optional<User.Gender> genderInType = Optional.ofNullable(gender)
                .map(g -> {
                    try {
                        return User.Gender.valueOf(g.toUpperCase());
        ...
        List<String> existingContacts = Optional.ofNullable(contactProxy.getContacts(pseudo, "ACTIVE").getBody()).orElse(Collections.emptyList())
                .stream()
        ...
        return allUsers.stream()
                ...
                .filter(user -> genderInType.isEmpty() || user.getGender() == genderInType.get())
